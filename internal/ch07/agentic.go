...
func migrateAgentic(ctx context.Context, deps internal.Deps) error {
	db := deps.DB
...
func runAgentic(ctx context.Context, deps internal.Deps, _ []string) error {
	...
	pc := splitter.SplitParentChild(l7Sample, parentCfg, childCfg)
	...
	if err := ch03.Ingest(ctx, deps.DB, deps.Embedder,
		infrastructure.Document{Title: "L7 sample", Lang: "zh"},
